#!/usr/bin/env bash
# Run Shorebird patch for every application that has shorebird.yaml, using the same
# compile-time API settings as ApiClient.fromDartDefines() in shared/api_client.
#
...
#
# Usage:
#   ./shorebird_patch_all.sh
#   ./shorebird_patch_all.sh --apps=kds,menu --platforms=android,ios
#   SHOREBIRD_PLATFORMS=android,ios,macos ./shorebird_patch_all.sh
#   MENU_BOARD_PLATFORMS=android,ios,macos,linux ./shorebird_patch_all.sh
#
...
#   SHOREBIRD_RELEASE_VERSION=1.0.0 ./shorebird_patch_all.sh
#   ENV_FILE=/path/to/other.env ./shorebird_patch_all.sh
#   ./shorebird_patch_all.sh --dry-run
#   ./shorebird_patch_all.sh --release-version=1.0.0
#
# Override the API base (must be https or http; host is derived for API_HOST):
#   PRODUCTION_API_BASE_URL='https://example.com/' ./shorebird_patch_all.sh
#
# Or set API_HOST / API_SECURE directly (skips URL parsing):
#   API_HOST='example.com' API_SECURE=true ./shorebird_patch_all.sh
...
# Which store release to patch (matches pubspec / release build). Use "latest" for the most recently updated release.
SHOREBIRD_RELEASE_VERSION="${SHOREBIRD_RELEASE_VERSION:-latest}"
...
if [[ "${SHOREBIRD_SCRIPT_HELP:-}" == "1" ]]; then
  cat <<'EOF'
Usage: shorebird_patch_all.sh [options] [-- extra shorebird args]
  ...
  --platforms=android,ios,macos      Patch these platforms for every selected app (overrides
                                     SHOREBIRD_PLATFORMS and MENU_BOARD_PLATFORMS for this run).

Environment: SHOREBIRD_PLATFORMS, MENU_BOARD_PLATFORMS, SHOREBIRD_RELEASE_VERSION, API_HOST, ENV_FILE, etc.

Examples:
  ./scripts/shorebird_patch_all.sh --apps=kds,menu --platforms=ios
  SHOREBIRD_RELEASE_VERSION=0.0.1+1 ./scripts/shorebird_patch_all.sh --apps=mobile
EOF
  exit 0
fi
...
echo "Shorebird patch — API_HOST=${API_HOST} API_SECURE=${API_SECURE} API_PORT=${API_PORT:-<empty>} API_KEY=${_api_key_echo}"
...
echo "Release version: ${SHOREBIRD_RELEASE_VERSION}"
echo ""
